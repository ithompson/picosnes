...
def get_keyed_comments_for_actions(cpu_data: CpuData) -> Dict[str, Dict[str, str]]:
    ...
    """
    return {action_name: {'pseudocode': pseudocode} for action_name, pseudocode in cpu_data.actions.items()}
...
def generate_rust_op_skeleton(cpu_data: CpuData, args):
    ...
    op_skeleton_content = op_skeleton_template.render(cpu_data=cpu_data, op_comments=op_comments)
...
def generate_rust_action_skeleton(cpu_data: CpuData, args):
    ...
    action_skeleton_content = action_skeleton_template.render(cpu_data=cpu_data, action_comments=action_comments)

    print(action_skeleton_content)

def get_keyed_rust_comments(file_path: Path, pattern: SgConfig) -> Dict[str, Dict[str, str]]:
    """
...
def validate_rust_code(cpu_data: CpuData, args):
    """
    ...
    action_comments = get_keyed_rust_comments(args.actions_file, RUST_ACTION_COMMENT_PATTERN)
        ...
        if op not in op_comments:
            ...
            for key, value in expected_comments.items():
                if key not in op_comments[op]:
                    ...
                    print(f"Mismatch for {key} in op {op}: expected '{value}', code currently has '{op_comments[op][key]}'")
        ...
        if action not in action_comments:
            ...
            for key, value in expected_comments.items():
                if key not in action_comments[action]:
                    ...
                    print(f"Mismatch for {key} in action {action}: expected '{value}', code currently has '{action_comments[action][key]}'")
...
def main() -> None:
    ...
    parser_rust_validate = subparsers.add_parser('rust_validate', help='Validate Rust op and action implementations against the CPU data file')
    parser_rust_validate.add_argument('--ops_file', type=Path, default=SCRIPT_DIR.parent / "host/nes_emu/src/components/cpu/ops.rs", help="Path to the Rust ops implementation file")
    parser_rust_validate.add_argument('--actions_file', type=Path, default=SCRIPT_DIR.parent / "host/nes_emu/src/components/cpu/sequences/actions.rs", help="Path to the Rust action implementation file")
