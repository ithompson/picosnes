...
class InstructionData(BaseModel):
    ...
    illegal: bool = False
    """Whether this instruction is an illegal instruction"""
    ...
    def validate_consistency(self):
        ...
        for sequence_name, sequence in all_sequences.items():
            for i, cycle in enumerate(sequence):
                assert cycle.action_name in self.actions, (
                    f"Invalid action '{cycle.action_name}' in {sequence_name}.{i}"
                )
                assert cycle.mem_cycle in VALID_MEM_CYCLES, (
                    f"Invalid memory cycle '{cycle.mem_cycle}' in {sequence_name}.{i}"
                )

        # Validate each instruction
        for opcode, inst_data in self.instructions.items():
            assert opcode >= 0 and opcode <= 0xFF, f"Invalid opcode 0x{opcode:02X}"
            assert inst_data.mnemonic in self.mnemonics, (
                f"Invalid mnemonic '{inst_data.mnemonic}' in opcode 0x{opcode:02X}"
            )
            assert inst_data.access_mode in self.access_modes, (
                f"Invalid access mode '{inst_data.access_mode}' in opcode 0x{opcode:02X}"
            )

            mnemonic_data = self.mnemonics[inst_data.mnemonic]
            assert (
                mnemonic_data.access_subtype
                in self.access_modes[inst_data.access_mode].sequences
            ), (
                f"Invalid access subtype '{mnemonic_data.access_subtype}' in opcode 0x{opcode:02X}"
            )
